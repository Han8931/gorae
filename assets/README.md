...
"ai": {
  ...
  // Let the model invoke in-app actions like save_markdown.
  // Requires a model that supports tool calling — see "Tool calling" below.
  "enable_tools": false
}
```
...
| `enable_tools` | Allow the model to call in-app tools (e.g. `save_markdown`). Default `false`. |
...
### In-chat keybindings (insert mode)

Insert mode is the default — typing flows into the prompt as you'd expect.
...
| `Esc` | Switch to **navigation mode** (or exit if the chat is empty) |
| `Ctrl+C` | Exit chat |
| `Ctrl+P` / `Ctrl+N` | Scroll chat up / down |
| `PgUp` / `PgDn` | Scroll chat half a page |
| Mouse wheel | Scroll chat up / down |

### Navigation mode (vim-style)

Press `Esc` while chatting to enter **navigation mode**, where you can move between past messages, copy them to the clipboard, and pick multiple at once. The current message gets a high-contrast role badge, the input prompt swaps for a bright `NORMAL` chip, and a hint line right above the global status bar lists the keys available. Press `i` to return to typing.
...
| `j` / `↓` | Next message |
| `k` / `↑` | Previous message |
| `l` / `→` | Jump to next user message (skip assistant turns) |
| `h` / `←` | Jump to previous user message |
| `gg`, `G` | First / last message |
| `Space` | Toggle a mark on the current message |
| `y` | Yank current message — or every marked message — to the system clipboard |
| `c` | Clear all marks |
| `/` | Re-enter insert mode prefilled with `/` for a slash command |
| `?` | Show in-chat help |
| `q` | Exit chat |

Yanking a single message copies its raw content (paste-ready, no `**You:**` / `**Gorae:**` wrappers). Yanking multiple marks joins them with `---` separators and prepends each with its role so context survives the paste.

> **New to vim modes?** The first time you press `Esc` per session, a one-shot status line explains the basics. If you accidentally land in normal mode and start typing, a safety-net hint tells you to press `i` to type or `q` to quit. `?` opens the full help.
...
### Tool calling (model-invoked actions)

With `"enable_tools": true` in your AI config, Gorae advertises a small set of in-app tools to the model. The model can then invoke them mid-conversation when the user asks for an action — no slash command required.
...
| Tool | What it does |
|---|---|
| `save_markdown` | Writes markdown to a file in `notes_dir`. The model passes the AI-generated content (summary, analysis, written-up answer) as `content` so the saved file is a clean document — not the chat transcript. If `content` is omitted, the full conversation is exported instead. |

A tool round-trip shows up inline in the chat as a muted block: `⚙ save_markdown(...)` followed by a green `↳ Saved to /path/...` reply (or red on error). These intermediate rounds are deliberately not persisted to the session DB, so resumed sessions show a clean Q&A thread.

> **Model support is uneven.** Tool calling works on `gpt-4o*`, recent Anthropic models behind OpenAI-compatible gateways, and Ollama with `llama3.1` / `qwen2.5`. Smaller local models silently ignore the `tools` field; flip `enable_tools` off if your model misbehaves.
